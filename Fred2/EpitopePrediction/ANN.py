...
try:
    class MHCNuggetsPredictor_class1_2_0(AANNEpitopePrediction):
        ...
        def predict(self, peptides, alleles=None, binary=False, **kwargs):
            ...
            print type(alleles)
            print type(alleles[0])
            print alleles[0].organism
            print len(alleles)
            print "before filtering"
            print alleles[0]
            print self.supportedAlleles
    ...
    class MHCNuggetsPredictor_class2_2_0(AANNEpitopePrediction):
        ...
        def revert_allele_repr(self, name):
            if name.startswith("H-2-"):
                ...
                if len(name_split) > 2:
                    return CombinedAllele(name_split[0] + "-" + name_split[1][:4] + '*' + name_split[1][4:] + '-' + name_split[2][:4] + '*' + name_split[2][4:])
                else:
